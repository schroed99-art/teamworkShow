...
## Phase 5.3 Vollausbau — Freier Zonen-Editor — GEPLANT (Umsetzung offen)
- **Ziel:** das feste Firma/Kunde-Split (v1.0.37) zu einem **frei im Dashboard authorbaren Zonenbaum** ausbauen — beliebig viele Zonen, geschachtelte Zeilen/Spalten mit freien Größen, jede Zone an eine Quelle gebunden, **eigenes Layout je Format** (portrait/landscape/phone/tablet). Nutzer-Entscheidungen: Layout je Format **ja**; Umfang **alles zusammen** (Engine + Editor + Vorschau).
- **Ansatz:** neue Spalte `devices.zone_layout` (JSON, Baum je Format), `zone_mode='custom'`; `playlist.php` löst den Baum rekursiv auf (`zones.tree`, Blätter tragen Slides via bestehendes `tw_slides_of`); App-Renderer `MainActivity.buildStages` von 2 festen `<include>`s auf rekursives Inflaten von `zone_stage` je Blatt (`stages: List<Stage>`); Dashboard-Editor (rekursives Teilen, Leinwand je Format) ersetzt `zoneFields`; Vorschau `pvOpen` wird rekursiver Baum-Renderer (`pvRenderNode`), geteilt mit der Editor-Leinwand. `single`/`split` bleiben als Rückfall.
- **Approved Plan:** `~/.claude/plans/perfekt-in-dieser-session-reactive-thimble.md` (Datenmodell, alle fünf Schichten, Verifikation, Reihenfolge). **Umsetzung bewusst für eine frische Session vorgesehen** (Kosten/Kontext). Reihenfolge: (1) Datenmodell+Migration+`devices.php` → (2) `playlist.php`-Resolver → (3) `SyncManager`+App-Renderer → (4) Editor → (5) Vorschau → (6) Verifikation.
